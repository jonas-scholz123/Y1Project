...
def mutate_children(children):
    for i in range(random.randint(0, 17), random.randint(18,35)):
        children[i] = children[i] * mutation_factor
#randomly generate a range betwen 0 and 36, use a for loop to scale the weights
...
def evolution(number_of_gens, generation, weights):
    ...
    children = breeding(weights, scores, 0.8/generation)
